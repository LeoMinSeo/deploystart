...
const MyPageReservation = ({ reservation, refreshData, uid }) => {
 ...
 <div className="flex justify-end ml-0 lg:ml-[1rem] mt-0 lg:mt-[0.5rem] min-h-[85vh] select-none">
   <div className="bg-white p-4 lg:p-8 rounded-lg shadow-lg lg:mt-20 w-full">
     <h2 className="text-2xl lg:text-3xl font-bold text-gray-800 mb-6 border-b pb-4 select-none border-gray-200">
       예매 내역
     </h2>
         ...
         {!reservation || reservation.length === 0 ? (
           ...
           <>
             {paginatedGroups.map(([date, items]) => (
               <div key={date} className="mb-6 lg:mb-8">
                 {/* 날짜 표시 */}
                 <div className="mb-3 lg:mb-4">
                   <div className="text-lg lg:text-xl font-bold text-gray-700 border-l-4 border-orange-400 pl-3">
                     {new Date(date)
                 ...
                 {items.map((item) => (
                   <div
                     key={item.ticketId}
                     className="mb-4 lg:mb-6 border p-3 lg:p-5 rounded-lg hover:shadow-md transition-shadow duration-200"
                   >
                     {/* 상태와 예약번호를 항상 같은 위치에 표시 */}
                     <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3">
                       <div
                         className={`flex items-center text-xs lg:text-sm font-semibold px-2 lg:px-3 py-1 mb-2 sm:mb-0 ${getStatusColor(
                           item.status
                         )}`}
                       >
                         {/* 상태 아이콘 추가 */}
                     ...
                     {/* 예약 상세 정보 */}
                     <div className="flex flex-col sm:flex-row items-start mt-2 p-2 lg:p-3 rounded-md">
                       <div className="flex-shrink-0 mb-3 sm:mb-0 sm:mr-5 w-full sm:w-auto flex justify-center sm:block">
                         <div
                           className={`w-24 sm:w-32 h-32 sm:h-44 bg-gray-100 flex items-center justify-center rounded-lg overflow-hidden shadow-sm
                           ${
                       ...
                       >
                         <div className="font-bold text-lg lg:text-xl mb-2 text-gray-800">
                           <Link
                         ...
                         <div className="flex flex-col text-gray-600 mb-3 space-y-1">
                         ...
                         <div className="flex justify-end mt-3 lg:mt-4">
                           ...
                           Refundable(item.concertStartTime) ? (
                             <button
                               ...
                               className="px-2 py-1 text-xs lg:text-sm text-red-500 bg-red-200 rounded-lg hover:bg-red-500 hover:text-white transition-colors active:scale-105"
                             >
                               예매 취소
                             </button>
                           ) : item.status === "RESERVATION" &&
             ...
             {/* 페이지네이션 */}
             {totalPages > 1 && (
               <div className="flex justify-center mt-6 lg:mt-8 overflow-x-auto">
                 <div className="flex space-x-1">
                   ...
                   {currentPage > 5 && (
                     <button
                       ...
                       className="px-2 lg:px-4 py-2 rounded-md bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors duration-200 text-sm"
                     >
                       &lt;
                     </button>
                   )}
                   ...
                   {Array.from(
                     ...
                     <button
                       ...
                       className={`px-2 lg:px-4 py-2 rounded-md transition-colors duration-200 text-sm ${
                         currentPage === number
